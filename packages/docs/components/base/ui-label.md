<script setup>
import { ref } from 'vue'
import UiLabel from '@praxis/ui-src/components/base/UiLabel.vue'
...
<ComponentDemo>
  <div style="width:300px;display:flex;flex-direction:column;gap:1.25rem">
    <div>
      <UiLabel label="Full Name" :for="inputId" />
      <input :id="inputId" class="input-base" placeholder="John Doe" style="margin-top:0.25rem" />
    </div>
    <div>
      <UiLabel label="Email Address" :for="inputIdReq" :required="true" />
...
<template>
  <!-- Basic label -->
  <UiLabel label="Full Name" for="name-input" />
  <input id="name-input" />

  <!-- Required indicator -->
  <UiLabel label="Email Address" for="email-input" :required="true" />
