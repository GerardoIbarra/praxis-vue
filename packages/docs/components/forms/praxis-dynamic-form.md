<script setup>
import { ref } from 'vue'
import PraxisDynamicForm from '@praxis/ui-src/components/forms/PraxisDynamicForm.vue'
...
<ComponentDemo>
  <div style="padding: 1rem 0; width: 100%;">
    <PraxisDynamicForm :schema="schema" v-model="formData" />
...
<script setup>
import { ref } from 'vue'
import { PraxisDynamicForm } from 'praxis-vue-ui'
...
> [!NOTE]
> `PraxisDynamicForm` is the most complex component in the library. See the schema type definitions in `@/types/api/common` for the full `FormSchemaField` interface, and `PraxisFormFieldRow` / `PraxisFormMultiSelectList` / `PraxisSelectListField` for the sub-field types used by the `row`, `multiselect_list`, and `select_list` field types.
