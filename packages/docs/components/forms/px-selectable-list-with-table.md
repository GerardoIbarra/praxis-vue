...
An extended version of `PxSelectableList` that displays selected items in a full `PxDataTable` instead of a simple list. Useful when the selected items need sortable columns, expandable rows, or custom cell rendering.
...
<PropsTable :rows="[
  ...
  { name: 'columns', type: 'ColumnDef[]', default: '[]', description: 'Column definitions for the PxDataTable (same as PxDataTable columns prop).' },
