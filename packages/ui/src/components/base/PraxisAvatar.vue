...
const shapeClass = computed(() =>
  props.shape === "square" ? "praxis-avatar--square" : "praxis-avatar--circle"
);
...
<template>
  <div class="praxis-avatar-wrapper" :class="{ 'praxis-avatar-has-label': label }">
    <div
      class="praxis-avatar"
    ...
    >
      ...
      <img
        ...
        :src="src"
        :alt="name || 'avatar'"
        class="praxis-avatar-img"
        @error="onImgError"
      />

      <!-- Initials fallback -->
      <span v-else class="praxis-avatar-initials">{{ initials }}</span>
    </div>

    <!-- Optional footer label -->
    <span v-if="label && name" class="praxis-avatar-label">{{ name }}</span>
  </div>
</template>

<style scoped>
.praxis-avatar-wrapper {
  display: inline-flex;
...
/* Shapes */
.praxis-avatar--circle { border-radius: 9999px; }
.praxis-avatar--square { border-radius: 0.5rem; }
