  ...
  <nav class="nav-list">
    ...
    <template v-for="item in model" :key="item.key ?? item.label">
      <div
        ...
        class="nav-list-separator"
        :class="{ 'nav-list-separator--collapsed': collapsed }"
      >
        <component :is="item.icon" v-if="item.icon" class="nav-list-separator-icon" />
        <span v-if="!collapsed" class="nav-list-separator-label">{{ item.label }}</span>
      </div>

      <div v-else-if="item.items" v-show="item.visible !== false" class="nav-list-group">
        <button
          type="button"
          class="nav-list-item"
          :class="{ 'nav-list-item--active': groupHasActiveChild(item) }"
          :title="collapsed ? item.label : undefined"
          @click="toggleGroup(item.key)"
        >
          <component :is="item.icon" v-if="item.icon" class="nav-list-item-icon" />
          <span v-if="!collapsed" class="nav-list-item-label">{{ item.label }}</span>
          <component
            :is="isExpanded(item.key) ? ChevronDown : ChevronRight"
            v-if="!collapsed"
            class="nav-list-item-chevron"
          />
        </button>

        <div v-if="isExpanded(item.key) && !collapsed" class="nav-list-subitems">
          <component
            :is="linkComponent"
            ...
            class="nav-list-item nav-list-item--sub"
            :class="{ 'nav-list-item--active': isActive(child.to) }"
            @click="onSelect(child)"
          >
            <component :is="child.icon" v-if="child.icon" class="nav-list-item-icon" />
            <span class="nav-list-item-label">{{ child.label }}</span>
          </component>
        </div>
      </div>
      ...
      <component
        ...
        class="nav-list-item"
        :class="{ 'nav-list-item--active': isActive(item.to) }"
        :title="collapsed ? item.label : undefined"
        @click="onSelect(item)"
      >
        <component :is="item.icon" v-if="item.icon" class="nav-list-item-icon" />
    ...
    <div v-if="$slots.footer" class="nav-list-footer">
      <slot name="footer" />
    </div>
  </nav>
</template>
...
.nav-list-separator--collapsed {
  justify-content: center;
}

.nav-list-separator-icon {
  width: 1rem;
  height: 1rem;
