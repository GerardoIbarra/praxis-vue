  ...
  <nav class="praxis-nav-list">
    ...
    <template v-for="item in model" :key="item.key ?? item.label">
      <div
        ...
        class="praxis-nav-list-separator"
        :class="{ 'praxis-nav-list-separator--collapsed': collapsed }"
      >
        <component :is="item.icon" v-if="item.icon" class="praxis-nav-list-separator-icon" />
        <span v-if="!collapsed" class="praxis-nav-list-separator-label">{{ item.label }}</span>
      </div>

      <div v-else-if="item.items" v-show="item.visible !== false" class="praxis-nav-list-group">
        <button
          type="button"
          class="praxis-nav-list-item"
          :class="{ 'praxis-nav-list-item--active': groupHasActiveChild(item) }"
          :title="collapsed ? item.label : undefined"
          @click="toggleGroup(item.key)"
        >
          <component :is="item.icon" v-if="item.icon" class="praxis-nav-list-item-icon" />
          <span v-if="!collapsed" class="praxis-nav-list-item-label">{{ item.label }}</span>
          <component
            :is="isExpanded(item.key) ? ChevronDown : ChevronRight"
            v-if="!collapsed"
            class="praxis-nav-list-item-chevron"
          />
        </button>

        <div v-if="isExpanded(item.key) && !collapsed" class="praxis-nav-list-subitems">
          <component
            :is="linkComponent"
            ...
            class="praxis-nav-list-item praxis-nav-list-item--sub"
            :class="{ 'praxis-nav-list-item--active': isActive(child.to) }"
            @click="onSelect(child)"
          >
            <component :is="child.icon" v-if="child.icon" class="praxis-nav-list-item-icon" />
            <span class="praxis-nav-list-item-label">{{ child.label }}</span>
          </component>
        </div>
      </div>
      ...
      <component
        ...
        class="praxis-nav-list-item"
        :class="{ 'praxis-nav-list-item--active': isActive(item.to) }"
        :title="collapsed ? item.label : undefined"
        @click="onSelect(item)"
      >
        <component :is="item.icon" v-if="item.icon" class="praxis-nav-list-item-icon" />
    ...
    <div v-if="$slots.footer" class="praxis-nav-list-footer">
      <slot name="footer" />
    </div>
  </nav>
</template>
...
.praxis-nav-list-separator--collapsed {
  justify-content: center;
}

.praxis-nav-list-separator-icon {
  width: 1rem;
  height: 1rem;
  color: var(--ui-text-muted, #64748b);
}

.praxis-nav-list-separator-label {
  font-size: 0.6875rem;
  font-weight: 600;
  color: var(--ui-text-muted, #64748b);
...
.praxis-nav-list-item:hover {
  background: var(--ui-bg-soft, #f8fafc);
}

.praxis-nav-list-item--active {
  background: var(--ui-primary, #2563eb);
  color: #fff;
  font-weight: 600;
}

.praxis-nav-list-item--sub {
  padding-left: 2rem;
}

.praxis-nav-list-item-icon {
  width: 1.25rem;
  height: 1.25rem;
  flex-shrink: 0;
}

.praxis-nav-list-item-label {
  flex: 1;
  overflow: hidden;
...
.praxis-nav-list-item-chevron {
  width: 1rem;
  height: 1rem;
  flex-shrink: 0;
}

.praxis-nav-list-footer {
  margin-top: auto;
  padding-top: 0.5rem;
